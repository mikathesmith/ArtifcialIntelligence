...
# data sample in 1D array x and corresponding target output y
(x, y) = dataset_reg.read(name="t4dataset1.npz")
# Show a scatter plot of the data
dataset_reg.show(input=x, output=y, type="scatter")

# This number indicates the percentage split for training data vs test data.  0.8 means that 80% of the data will
...
# Polynomial hypothesis. Instantiates the polynomial class, providing function for computing output and training
# using steepest gradient descent, of a polynomial hypothesis of degree k
#h = polynomial(k=3)
#k=3, a=0.05

 # Coscos hypothesis
#h = coscos(k=4)
#change cos to sin and works fine! k=4, a=0.01

# MLP hypothesis, increase a to 0.1
h = mlp(layers=[{'neurons': 3, 'activation': 'sigmoid'},
                {'neurons': 3, 'activation': 'sigmoid'},
                {'neurons': 2, 'activation': 'lin'}
                ])
...
# Runs the hypothesis on training data, prints the MSE cost after each iteration, displays visualisation
# of its fit every 100 iterations.
for epoch in range(20000):
    ...
    J_train = h.learn(input=x_train, true_output=y_train, alpha=0.05)
